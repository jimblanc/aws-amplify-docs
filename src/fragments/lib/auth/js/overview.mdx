...
For many apps, user sign-up and sign-in is all that is required. Once authenticated the app can talk to an API to access and mutate data. In this case, you can simply create a User Pool by running `amplify add auth` using the Amplify CLI and selecting the default setup. In your application you can use `signUp` and `signIn` (or an Amplify UI component) APIs to complete this process and retrieve tokens. The Amplify client will refresh the tokens calling `fetchAuthSession` if they are no longer valid.

 

## Social Provider Federation

Many apps also support login with social providers such as Facebook, Google Sign-In, or Login With Amazon. The preferred way to do this is via an OAuth redirect which lets users login using their social media account and a corresponding user is created in User Pools. With this design you do not need to include an SDK for the social provider in your app. Set this up by running `amplify add auth` and selecting the social provider option. Upon completion you can use `signInWithRedirect` in your application to either show a pre-built "Hosted UI" or pass in a provider name (e.g. `signInWithRedirect({provider: 'Facebook'})`) to interface directly and build out your own UI.

 

You can also get credentials directly from Identity Pools by passing tokens from a provider directly to `signInWithRedirect`. However you will have to use that provider's SDK directly in your app and manage token refresh and auth flows manually.
