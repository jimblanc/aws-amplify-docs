...
Before you can add an existing AWS resource to an Android application, the application must have the Amplify libraries installed. For detailed instructions, see [Install Amplify Libraries](/[platform]/start/project-setup/create-application/#n2-install-amplify-libraries).
...
* See the [Analytics category](/[platform]/build-a-backend/more-features/analytics/existing-resources/) to use existing AWS Pinpoint resources.
* See the [API (GraphQL) category](/[platform]/build-a-backend/graphqlapi/existing-resources/) to use existing AWS AppSync resources.
* See the [API (REST) category](/[platform]/build-a-backend/restapi/existing-resources/) to use existing Amazon API Gateway and AWS Lambda resources.
* See the [Authentication category](/[platform]/build-a-backend/auth/set-up-auth/#prerequisites) to use existing Amazon Cognito resources.
* See the [Storage category](/[platform]/build-a-backend/storage/existing-resources/) to use existing Amazon S3 resources.
